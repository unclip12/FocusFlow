...
import { initDB, getCachedData, cacheFirebaseData } from './services/offlineStorage'; // 🆕 Offline Storage
import { 
...
export default function App() {
  ...
  const loadTodayPlan = useCallback(async () => {
      try {
          ...
          setTodayPlan(plan);
          // 🆕 Cache today's plan for instant load next time
          if (plan) {
              await cacheFirebaseData('todayPlan', plan);
          }
      } catch (e) {
    ...
    const unsubscribe = auth.onAuthStateChanged(async (u) => {
      ...
      if (u) {
        try {
            ...
            // 🆕 OFFLINE CACHING: Load from cache first for instant UI
            ...
            if (firestoreKB) {
                ...
                // 🆕 OFFLINE CACHING: Cache the fresh data for next time
            ...
            if (fmge) {
                setFmgeData(fmge);
                // 🆕 Cache FMGE data
                await cacheFirebaseData('fmgeData', fmge);
            }
  ...
  const updateKB = useCallback(async (newKB: KnowledgeBaseEntry[]) => {
      ...
      // 🆕 Update cache whenever KB changes
      await cacheFirebaseData('knowledgeBase', newKB);
  }, []);

  const handleUpdateFMGE = useCallback(async (entry: FMGEEntry) => {
      const updated = fmgeData.filter(e => e.id !== entry.id).concat(entry);
      setFmgeData(updated);
      await saveFMGEEntry(entry);
      // 🆕 Update cache
      await cacheFirebaseData('fmgeData', updated);
  }, [fmgeData]);

  const handleDeleteFMGE = useCallback(async (id: string) => {
      const updated = fmgeData.filter(e => e.id !== id);
      setFmgeData(updated);
